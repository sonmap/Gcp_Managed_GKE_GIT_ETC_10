...
RESOURCE_ADDRESS='google_cloud_run_v2_service.provisioner[0]'
...
BACKUP_STATE="/tmp/${DEPLOYMENT_ID}-before-cloud-run-untaint-$(date +%Y%m%d%H%M%S).tfstate"
LOCK_ID=""

cleanup() {
  local exit_code=$?
  if [[ -n "${LOCK_ID}" ]]; then
    gcloud infra-manager deployments unlock "${DEPLOYMENT_ID}" \
      --project="${PROJECT_ID}" \
      --location="${LOCATION}" \
      --lock-id="${LOCK_ID}" >/dev/null || true
  fi
  rm -r -- "${WORK_DIR}"
  exit "${exit_code}"
}
...
gcloud infra-manager deployments lock "${DEPLOYMENT_ID}" \
  --project="${PROJECT_ID}" \
  --location="${LOCATION}"

LOCK_ID="$(gcloud infra-manager deployments export-lock "${DEPLOYMENT_ID}" \
  --project="${PROJECT_ID}" \
  --location="${LOCATION}" \
  --format='value(lockId)')"

STATE_URL="$(gcloud infra-manager deployments export-statefile "${DEPLOYMENT_ID}" \
  --project="${PROJECT_ID}" \
  --location="${LOCATION}" \
...
CURRENT_STATUS="$(jq -r '
  .resources[]
  | select(.type == "google_cloud_run_v2_service" and .name == "provisioner")
  | .instances[0].status // "managed"
' "${WORK_STATE}")"

if [[ "${CURRENT_STATUS}" != "tainted" ]]; then
  echo "Unexpected Cloud Run state: status=${CURRENT_STATUS}" >&2
...
UPLOAD_URL="$(gcloud infra-manager deployments import-statefile "${DEPLOYMENT_ID}" \
  --project="${PROJECT_ID}" \
  --location="${LOCATION}" \
  --lock-id="${LOCK_ID}" \
  --format='value(signedUri)')"
...
echo "Cloud Run state recovery completed."
