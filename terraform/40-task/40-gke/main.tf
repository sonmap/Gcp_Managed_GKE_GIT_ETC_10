...
variable "gke_project_id" { type = string; default = "gcp-sbx-edp-gke01" }
variable "cluster_name" { type = string; default = "gke-sbx-edp-main-an3" }
variable "location" { type = string; default = "asia-northeast3" }
variable "task_name" { type = string }
variable "jupyter_gsa_email" { type = string }
data "google_client_config" "current" {}
data "google_container_cluster" "main" { project = var.gke_project_id; name = var.cluster_name; location = var.location }
provider "kubernetes" {
...
resource "kubernetes_resource_quota_v1" "task" {
  metadata { name = "quota-${var.task_name}"; namespace = kubernetes_namespace_v1.task.metadata[0].name }
...
resource "kubernetes_network_policy_v1" "default_deny_ingress" {
  metadata { name = "default-deny-ingress"; namespace = kubernetes_namespace_v1.task.metadata[0].name }
  spec { pod_selector {}; policy_types = ["Ingress"] }
}
